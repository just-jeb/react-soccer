...
export const NextMovesGridComponent = React.memo<NextMovesGridProps>(({nextMoves, onClick, hintColor}) => {
    return (<g>
        {
            nextMoves.map(({coordinates: {x, y}, id}) =>
                <g key={id} transform={`translate(${x - 0.5}, ${y - 0.5})`} onClick={() => onClick(id)}>
