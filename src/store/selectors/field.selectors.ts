...
export const ballNodeSelector = (state: IState) =>
  getBallNode(state.fieldState);
...
export const pathCoordinatesSelector = createSelector(
  ...
  pathSelector,
  (nodes, path) =>
    path.reduce<TConnectionCoords[]>((coordPairs, [id1, id2]) => {
    ...
    for (const predicate of mutualExclusionPredicates) {
      if (predicate(coordinates)) {
        possibleMoves = possibleMoves.filter(
          ({ coordinates: coords }) => !predicate(coords),
        );
      }
    }
